...
class Message(object):
    ...
    def __init__(self, msg_type, subject,
                 packed_body=None,
                 headers=None,
                 packed_headers=None,
                 msg_id=None,
        ...
        if headers and packed_headers:
            raise TypeError(
                "Message takes either 'headers' or 'packed_headers' not both")
        ...
        if 'body' in kwargs:
            if packed_body is not None:
                raise TypeError(
                    "Message takes either 'body' or 'packed_body' not both")
    ...
    def pack_frames(self):
        return [self.id.encode('utf-8'), self.type,
                self.subject.encode('utf-8'), self.packed_headers,
                self.packed_body,]
    ...
    def unpack_frames(self, frames):
        try:
            ...
            raise ValueError(
                'bad message frame count: got %s, expected 6' % len(frames))
        ...
        try:
            ...
            raise ValueError(
                'message id, subject, and source must be utf-8 encoded.')
    ...
    def __str__(self):
        return '{type=%s subject=%s id=%s..}' % (self.type, self.subject,
                                                 self.id[:10],)

    def __repr__(self):
        return '<Message id=%s type=%s subject=%s body=%r>' % (
            self.id, self.type, self.subject, self.body,)
