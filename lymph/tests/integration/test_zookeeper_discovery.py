...
class ZookeeperIntegrationTest(LymphIntegrationTestCase):
    ...
    def test_lookup(self):
        ...
        self.assertEqual(list(service)[0].endpoint,
                         self.upper_container.endpoint)

    def test_upper(self):
        reply = self.lymph_client.request('upper', 'upper.upper',
                                          {'text': 'foo'})
        self.assertEqual(reply.body, 'FOO')

    def test_ping(self):
        reply = self.lymph_client.request('upper', 'lymph.ping',
                                          {'payload': 42})
    ...
    def test_connection_loss(self):
        service = self.lymph_client.container.lookup('upper')
        self.assertEqual([i.identity for i in service],
